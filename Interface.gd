...
func set_ability_cooldown(ability, cooldown):
	if ability == -1: #dash
...
func set_ability_on_cooldown(ability):
	if ability == -1: #dash
		dash_cd_bar.value = dash_cd_bar.max_value
	elif ability == 1: #charged shot
		ability1_cd_bar.value = ability1_cd_bar.max_value
	elif ability == 2: #ground explosion
		ability2_cd_bar.value = ability2_cd_bar.max_value
