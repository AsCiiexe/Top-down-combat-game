...
func _physics_process(delta):
	...
	if player != null:
		max_camera_distance = get_viewport_rect().size * 0.4
		camera_position = player.get_local_mouse_position() * 0.45
		...
		camera.position = lerp(camera.position, camera_position, 0.30)
